...
def train_task(task: str, config: dict, output_dir: Optional[Path] = None,
    ...
    # Save SSM core params BEFORE save_model() — PEFT's save_pretrained() sets
    # inference_mode=True which resets requires_grad=False on non-LoRA params,
    # making them undetectable by a requires_grad filter afterward.
    import torch
    ...
    if ssm_state:
        ...
        logger.warning("No SSM core params found! Check model.named_parameters() for expected patterns.")
