...
CONFIG = {
    ...
    "lora_alpha": 32,
    "lora_dropout": 0.05,
    # V1 BASELINE — wrong targets that match standard transformers, NOT Granite hybrid.
    # gate_proj/up_proj/down_proj don't exist in this architecture.
    # Only hits 4 attention layers (32 tensors, 1.4% of model).
...
def snapshot_ssm_params(model, label: str = "before") -> dict:
    """Snapshot A_log, D, dt_bias from first 3 Mamba layers for before/after comparison."""
    import torch
    snapshot = {}
    ...
    for name, val in sorted(snapshot.items()):
        print(f"  {name}: mean={val.float().mean():.6f}, std={val.float().std():.6f}, min={val.float().min():.6f}, max={val.float().max():.6f}")
    print()
    return snapshot


def compare_ssm_snapshots(before: dict, after: dict, output_dir: Path):
    """Compare before/after SSM snapshots and save delta report."""
    import torch
    lines = []
    lines.append("=" * 60)
    ...
    for name in sorted(before.keys()):
        if name not in after:
            continue
        b, a = before[name].float(), after[name].float()
        delta = (a - b).abs()
        mean_delta = delta.mean().item()
        max_delta = delta.max().item()
        pct_changed = (delta > 1e-6).float().mean().item() * 100

        status = "UPDATED ✓" if pct_changed > 0.5 else "UNCHANGED ✗"
        if pct_changed > 0.5:
            any_updated = True
        ...
        lines.append(f"    delta:  mean={mean_delta:.8f}, max={max_delta:.8f}, changed={pct_changed:.1f}%  [{status}]")

    lines.append("\n" + "-" * 60)
    if any_updated:
        lines.append("RESULT: SSM core parameters UPDATED during training ✓")
    else:
        lines.append("RESULT: SSM core parameters UNCHANGED during training ✗")
        lines.append("(Expected for V1 baseline — SSM params not unfrozen)")

    report = "\n".join(lines)
    print(report)

    report_path = output_dir / "ssm_parameter_delta.txt"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(report)
    print(f"\nSaved SSM delta report to {report_path}")
...
def train_task(task: str, config: dict, output_dir: Optional[Path] = None,
    ...
    # Snapshot SSM params before training (control — should not change in V1)
    ssm_before = snapshot_ssm_params(model, "before")
    ...
    # Snapshot SSM params after training and compare
    ssm_after = snapshot_ssm_params(model, "after")
    compare_ssm_snapshots(ssm_before, ssm_after, output_dir / run_name)
